...
import { evaluateChapter4 } from "@/lib/game/evaluate/chapter-4";
...
function OptionButton({
  ...
  const colorClass = () => {
    if (!hasAnswered)
      return "border-white/20 bg-white/10 hover:bg-white/20 text-white";
    if (option.isCorrect)
      return "border-green-400 bg-green-500/30 text-green-200";
...
export default function GamePage() {
  ...
  const gameResult = evaluateChapter4(score, game_4.length);
    ...
    return (
      ...
      >
        <ScoreModal
          ...
          onNext={() => router.push("/chapters/chapter-5")}
        />
      </div>
    );
  }
      ...
      <div className="relative z-20 flex items-center justify-between p-4 pt-5">
        ...
        <div className="flex gap-1.5 items-center">
          {game_4.map((_, i) => (
            <div
              key={i}
              className={`h-2 rounded-full transition-all duration-300 ${i === sceneIndex ? "w-6 bg-white" : i < sceneIndex ? "w-2 bg-white/70" : "w-2 bg-white/25"}`}
            />
          ))}
        </div>
        <span className="text-white/80 text-sm font-medium">
          {sceneIndex + 1}/{game_4.length}
        </span>
      </div>
          ...
          <div className="flex-1 flex flex-col justify-end pb-4 px-3 md:px-4 md:pb-6 w-full">
            ...
            >
              ...
              <div className="px-4 pt-3 pb-1 border-b border-white/10">
                <span className="font-jaro text-pink-300 text-base md:text-lg tracking-wide">
                  Situasi
                </span>
              </div>
              ...
              <div className="p-4 flex flex-col gap-3 h-37.5 sm:h-auto sm:max-h-[40vh] overflow-y-auto">
                {!hasAnswered ? (
                  ...
                  <>
                    {/* Feedback */}
                    <div
                      className={`rounded-xl px-4 py-3 text-sm md:text-base leading-relaxed border ${
                        answerState === "correct"
