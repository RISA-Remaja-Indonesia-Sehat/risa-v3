...
export default function PostTestFlower({
  status,
  highlight = false,
}: PostTestFlowerProps) {
  const content = (
    ...
    >
      <Image
        ...
        className={`
          ...
          ${
            highlight
              ? "brightness-110 drop-shadow-[0_0_24px_rgba(250,204,21,0.95)]"
              : ""
          }
        `}
      />

      {/* Tulisan di tengah bunga */}
      <div
        className="
          absolute
          left-1/2
  ...
  if (status === "locked") {
    return (
      <button
        id="post-test-ftue-target"
        type="button"
        disabled
        aria-label="Post Test belum tersedia"
        className={`
          ${commonClassName}
